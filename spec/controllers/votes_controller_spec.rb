...
RSpec.describe VotesController, type: :controller do
 ...
 let(:my_comment) {Vote.create!(valule: 1)}
 ...
 context "guest" do 
 	...
 	describe "POST down_vote" do 
 		it "redirect the user to the sign-in view with down_vote" do
 ...
 context "signed in user" do 
 	...
 	describe "POST up_vote" do 
 		...
 		it "user's second vote does not increase by one" do 
 			post :up_vote, post_id: user_post.id
 			votes = user_post.votes.count
 			post :up_vote, post_id: user_post.id
 		...
 		it "increases the sum of the post by one" do 
 		...
 		it ":back redirects to posts show page" do 
 			request.env["HTTP_REFERER"] = topic_post_path(my_topic, user_post)   # test to ensure that users are redirected back to the correct view
 			post :up_vote, post_id: user_post.id
 	...
 	describe "POST down_vote" do
 		it "the users first vote decrease number of post votes by one" do
 			votes = user_post.votes.count
 			post :down_vote, post_id: user_post.id
 			expect(user_post.votes.count).to eq(votes +1)
 		end

 		it "user's second votes does not decrease by one" do 
 			post :down_vote, post_id: user_post.id
 			votes = user_post.votes.count
 			post :down_vote, post_id: user_post.id
 			expect(user_post.votes.count).to eq(votes)
 		...
 		it "decrease the sume of the post by one" do
 			points = user_post.points
 			post :down_vote, post_id: user_post.id
 			expect(user_post.points).to eq(points - 1)
 		end
